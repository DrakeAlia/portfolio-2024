...
export function Analytics({ googleAnalyticsId, plausibleDomain }: AnalyticsProps) {
  ...
  useEffect(() => {
    ...
    const handleContactClick = (e: Event) => {
      const target = e.target as HTMLElement;
      const contactLink = target.closest('a[href^="mailto:"], a[href*="github.com"], a[href*="twitter.com"], a[href*="linkedin.com"]');
      if (contactLink) {
    ...
    const handleResumeDownload = (e: Event) => {
      const target = e.target as HTMLElement;
      if (target.closest('a[href*=".pdf"]') || target.textContent?.toLowerCase().includes('resume')) {
